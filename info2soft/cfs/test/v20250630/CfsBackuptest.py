...
from info2soft.cfs.v20250630.CfsBackup import CfsBackup
...
class CfsBackupTestCase(unittest.TestCase):
    ...
    def testFailoverCfsBackup(self):
        a = Auth(username, pwd)
        body = {
            'rep_uuids': [],
            'operate': '',
        ...
        cfsBackup = CfsBackup(a)
        r = cfsBackup.failoverCfsBackup(body)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'CfsBackup', 'failoverCfsBackup', body)
    ...
    def testDescribeCfsBackupLetency(self):
        a = Auth(username, pwd)
        body = {
            'rep_uuid': '',
            'start_time': 1,
        ...
        cfsBackup = CfsBackup(a)
        r = cfsBackup.describeCfsBackupLetency(body)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'CfsBackup', 'describeCfsBackupLetency', body)

    def testCheckCfsBackupCachedData(self):
        a = Auth(username, pwd)
        body = {
            'rep_uuid': '',
        }
        ...
        cfsBackup = CfsBackup(a)
        r = cfsBackup.checkCfsBackupCachedData(body)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'CfsBackup', 'checkCfsBackupCachedData', body)
