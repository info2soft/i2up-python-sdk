...
from info2soft.resource.v20250630.DtoStorageBucket import DtoStorageBucket
...
class DtoStorageBucketTestCase(unittest.TestCase):
    ...
    def testModifyDtoStorageBucket(self):
        a = Auth(username, pwd)
        body = {
            'quota_size': '',
            'bucket_uuid': '',
        ...
        dtoStorageBucket = DtoStorageBucket(a)
        r = dtoStorageBucket.modifyDtoStorageBucket(body, uuid)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'DtoStorageBucket', 'modifyDtoStorageBucket', body)
