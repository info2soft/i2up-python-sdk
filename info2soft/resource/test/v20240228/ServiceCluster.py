...
import unittest
from info2soft.resource.v20240228.ServiceCluster import ServiceCluster
...
class ServiceClusterTestCase(unittest.TestCase):
    ...
    def testModifyServiceCls(self):
        ...
        serviceCluster = ServiceCluster(a)
        uuid = 'AB790B28-62D6-7236-612F-65D73C80036F'
        r = serviceCluster.modifyServiceCls(body, uuid)
        print(r[0])
        assert r[0]['ret'] == 200
    ...
    def testConfigServiceCls(self):
        ...
        serviceCluster = ServiceCluster(a)
        r = serviceCluster.configServiceCls(body)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'ServiceCluster', 'configServiceCls', body)

    def testDescribeServiceClsConfig(self):
        a = Auth(username, pwd)
        body = {
            'cls_uuid': '',
        }
        ...
        serviceCluster = ServiceCluster(a)
        r = serviceCluster.describeServiceClsConfig(body)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'ServiceCluster', 'describeServiceClsConfig', body)

    def testListServiceClsValidNode(self):
        a = Auth(username, pwd)
        body = {
            'cls_uuid': '',
        }
        ...
        serviceCluster = ServiceCluster(a)
        r = serviceCluster.listServiceClsValidNode(body)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'ServiceCluster', 'listServiceClsValidNode', body)
