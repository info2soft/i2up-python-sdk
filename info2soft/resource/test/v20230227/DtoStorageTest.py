...
import unittest
from info2soft.resource.v20230227.DtoStorage import DtoStorage
# from info2soft.resource.v20200722.DtoStorage import DtoStorage
...
class DtoStorageTestCase(unittest.TestCase):
    ...
    def testCreateBucket(self):
        ...
        dtoStorage = DtoStorage(a)
        r = dtoStorage.createBucket(body)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'DtoStorage', 'createBucket', body)

    def testGetDtoStorageStatus(self):
        a = Auth(username, pwd)
        body = {
            'uuids': [],
        }
        ...
        dtoStorage = DtoStorage(a)
        r = dtoStorage.getDtoStorageStatus(body)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'DtoStorage', 'getDtoStorageStatus', body)
