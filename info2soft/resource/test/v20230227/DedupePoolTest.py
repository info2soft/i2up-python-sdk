...
import unittest
from info2soft.resource.v20230227.DedupePool import DedupePool
...
class DedupePoolTestCase(unittest.TestCase):
    ...
    def testOperateDedupePool(self):
        a = Auth(username, pwd)
        body = {
            'pool_uuids': [],
            'operate': '',
        ...
        dedupePool = DedupePool(a)
        r = dedupePool.operateDedupePool(body)
        print(r[0])
        assert r[0]['ret'] == 200
        write(r[0], 'DedupePool', 'operateDedupePool', body)
